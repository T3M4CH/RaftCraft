...
{
    public class StormTrooper : HumanBase
    {
        #region Fields

        [SerializeField] private AnimatorHelperEvent _animatorHelper;
        
        private IAction _moveRopeBehavior;
        ...
        private readonly int _hangHash = Animator.StringToHash("Hang");
        ...
        protected override void EnterAction()
        {
            ...
            _animator.CrossFade(_hangHash, Random.Range(0f, 0.2f), 0);
        }
        ...
        public override void Command(ICommand command)
        {
            switch (command)
            {
                ...
                case Move move:
                    ...
                    break;
                case MoveRope moveRope:
                    CurrentState = HumanState.Action;
                    var target = moveRope.Target;
                    _moveRopeBehavior = new ActionHang(target, transform, _speedMoveModified);
                    break;
                default:
                    CurrentState = HumanState.Movement;
                    break;
